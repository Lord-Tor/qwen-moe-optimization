...
def main():
    ...
    for fp in sorted(glob.glob(f"{args.scan_dir}/{MN}_*_costscan.jsonl")):
        subj = os.path.basename(fp).replace(f"{MN}_", "").replace("_costscan.jsonl", "")
        cost[subj] = load_scan(fp)
    math = {}
    for fp in sorted(glob.glob(f"{args.scan_dir}/{MN}_*_scan.jsonl")):
        subj = os.path.basename(fp).replace(f"{MN}_", "").replace("_scan.jsonl", "")
    ...
    print("=" * 90)
    hdr = "domain".ljust(28) + " | " + " | ".join(f"x{m}".ljust(7) for m in mults)
    print(hdr); print("-" * len(hdr))
    for subj, d in cost.items():
        line = subj.ljust(28) + " | " + " | ".join(
    ...
    for subj, d in math.items():
        ms = sorted(d); ax.plot(ms, [d[m] for m in ms], marker="o", lw=2,
                                color="tab:blue", alpha=0.5,
                                label="MATH (выигрыш)" if subj == list(math)[0] else None)
    for subj, d in cost.items():
        ms = sorted(d); ax.plot(ms, [d[m] for m in ms], marker="s", ls="--",
                                label=f"{subj} (цена)")
    ax.axhline(0, color="gray", lw=1)
    ax.set_xscale("log")
    ax.set_xlabel("bias multiplier (log)")
    ax.set_ylabel("Δ accuracy vs baseline (п.п.)")
    ax.set_title(f"{MN}: trade-off — math выигрыш (синие) vs non-math цена (пунктир)")
